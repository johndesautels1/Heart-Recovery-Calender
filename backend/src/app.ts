...
// Rate limiting configuration (DEVELOPMENT MODE - Generous limits)
...
const authLimiter = rateLimit({
  windowMs: 15 * 60 * 1000, // 15 minutes
  max: 50, // Limit each IP to 50 login requests per windowMs (increased from 5)
...
const uploadLimiter = rateLimit({
  ...
  max: 200, // Limit each IP to 200 uploads per hour (increased from 20)
