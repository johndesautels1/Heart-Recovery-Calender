...
// ALL exercise tips - comprehensive database
const TIPS_DB = {
...
// For each exercise in TIPS_DB, find and add the tips
let updatedCount = 0;
for (const [exerciseName, tips] of Object.entries(TIPS_DB)) {
  // Create the pattern to find this exercise
  const escapedName = exerciseName.replace(/[()]/g, '\\$&').replace(/'/g, "\\'");

  // Pattern: find the exercise, check if it doesn't already have formTips
  const pattern = new RegExp(
    `(\\{\\s+name: '${escapedName}',[\\s\\S]*?isActive: true,(?:\\s+videoUrl: '[^']*',)?(?:\\s+imageUrl: '[^']*',)?)\\s+(\\},)`,
    'm'
  );

  const match = content.match(pattern);
  if (match && !match[1].includes('formTips:')) {
    // Add the tips before the closing brace
    const formTips = tips.formTips.replace(/'/g, "\\'");
    const modifications = tips.modifications.replace(/'/g, "\\'");

    const replacement = `${match[1]}\n    formTips: '${formTips}',\n    modifications: '${modifications}',\n  ${match[2]}`;
    content = content.replace(pattern, replacement);
    updatedCount++;
    console.log(`✓ Added tips to: ${exerciseName}`);
  } else if (match && match[1].includes('formTips:')) {
    console.log(`- Already has tips: ${exerciseName}`);
  } else {
    console.log(`✗ Could not find: ${exerciseName}`);
  }
}
...
console.log(`\n✅ Successfully updated ${updatedCount} exercises with formTips and modifications!`);
console.log(`\nNext step: Run the seed script with:`);
console.log(`npx ts-node src/scripts/seedExercises.ts`);
